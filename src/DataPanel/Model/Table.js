...
class Table {
    ...
    /**
     * data is an object of all the data keyed by a unique id
     * columns is an array of all the columns
     * columnType is a object, where each column is a key to its type, e.g. 'string', 'int', 'numeric'
     */
    constructor(
        globalKey, updateGlobalStore, // vars related to global store
        data, columns, columnTypes
    ) {
        this.updateGlobalStore = updateGlobalStore;
        this.globalKey = globalKey;

        if(data || columns || columnTypes){
            this.updateTable(data, columns, columnTypes);
        }else{
            //show dummy data
            this.dummyData();
        }
